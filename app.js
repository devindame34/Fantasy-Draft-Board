...
].map((p,i)=>({id:`starter-${i+1}`,name:p[0],pos:p[1],team:p[2],tier:p[3],sleeper:p[4],myGuy:false,risky:false,favorite:false,status:"available"}));
    ...
    if(saved && Array.isArray(saved.players)){
      ...
      if(saved.players.length !== starterPlayers.length){
        ...
        saved.players = starterPlayers.map(p => {
          ...
          return old ? {...p, sleeper:old.sleeper ?? p.sleeper, myGuy:!!old.myGuy, risky:!!old.risky, favorite:!!old.favorite, status:old.status || "available"} : p;
        });
      }
  ...
  return `<article class="player ${statusClass}" data-id="${esc(p.id)}">
    ...
    <div class="player-main open-sheet">
      ...
      <div class="meta">
        ...
        ${p.sleeper?`<span class="tag tag-sleeper">SLEEPER</span>`:""}
        ${p.myGuy?`<span class="tag tag-myguy">MY GUY</span>`:""}
        ${p.risky?`<span class="tag tag-risky">RISKY</span>`:""}
...
function matchesFilters(p){
  ...
  const tagMatch =
    tagFilter==="ALL" ||
    (tagFilter==="SLEEPER" && p.sleeper) ||
    (tagFilter==="MYGUY" && p.myGuy) ||
    (tagFilter==="RISKY" && p.risky);
...
function openSheet(){
  ...
  $("#toggleSleeperBtn").textContent = p.sleeper ? "✓ Sleeper" : "Sleeper";
  $("#toggleMyGuyBtn").textContent = p.myGuy ? "✓ My Guy" : "My Guy";
  $("#toggleRiskyBtn").textContent = p.risky ? "✓ Risky / Fade" : "Risky / Fade";
...
$("#toggleSleeperBtn").onclick=()=>toggleTag("sleeper");
$("#toggleMyGuyBtn").onclick=()=>toggleTag("myGuy");
$("#toggleRiskyBtn").onclick=()=>toggleTag("risky");
