...
function makeState(raw) {
  ...
  return {
    boardVersion: window.BOARD_VERSION || "6.2.0",
...
function saveState() {
  state.boardVersion = window.BOARD_VERSION || "6.2.0";
  try {
...
function makeItBackEstimate(player) {
  ...
  if (player.adpConfidence) confidence = player.adpConfidence;
  else if (player.marketSource === "expert-proxy") confidence = "Low";
...
function bestCandidates() {
  ...
  if ($("#positionFilter").value !== "ALL" || $("#teamFilter").value !== "ALL") {
    return ranked.slice(0, 30).map(item => item.player);
  }
...
function marketSourceLabel(player) {
  if (player.marketSource === "multi-site-adp") {
    const count = Number(player.adpSourceCount) || 0;
    return `${count} managed-league platform${count === 1 ? "" : "s"} • median`;
  }
...
function rebuildTeamFilterOptions() {
  ...
  const teams = [...new Set(MASTER_PLAYERS.map(player => player.team).filter(Boolean))].sort();
  select.innerHTML = '<option value="ALL">All teams</option>' + teams
    .map(team => `<option value="${esc(team)}">${esc(team)}</option>`)
    .join("");
  select.value = teams.includes(previous) ? previous : "ALL";
}
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => navigator.serviceWorker.register("./sw.js?v=6.2.0").catch(() => {}));
}
