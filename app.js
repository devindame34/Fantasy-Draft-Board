...
function matchesFilters(player) {
  ...
  return textMatch && positionMatch && teamMatch && tagMatch;
}

function populateTeamFilter() {
  const select = $("#teamFilter");
  const teams = [...new Set(MASTER_PLAYERS.map(player => String(player.team || "").trim()).filter(Boolean))].sort();
  select.replaceChildren(new Option("All teams", "ALL"));
...
function playerCard(player) {
  ...
  return `<article class="player ${stateClass}" data-id="${esc(player.id)}">
    ...
    <div class="player-main open-sheet" role="button" tabindex="0">
      ...
      <div class="meta"><span class="pos">${esc(player.pos)}</span><span class="team">${esc(player.team || "FA")}</span><span class="market-chip">${esc(marketBadge(player))}</span><span class="value-badge">VALUE ${value}</span></div>
...
function draftLogRow(player) {
  ...
  return `<div class="draft-log-row player" data-id="${esc(player.id)}">
    ...
    <div class="player-main open-sheet" role="button" tabindex="0">
      ...
      <div class="meta"><span class="pos">${esc(player.pos)}</span><span class="team">${esc(player.team || "FA")}</span><span class="status-badge ${player.status === "mine" ? "mine-badge" : "taken-badge"}">${player.status === "mine" ? "MY PICK" : "TAKEN"}</span></div>
    </div>
    <button class="quick return-quick" type="button">RETURN</button>
  </div>`;
}
...
function openSheet() {
  ...
  $("#sheetPlayer").innerHTML = `
    <div class="detail-title-row"><div><div class="detail-title">${esc(player.name)}</div><div class="detail-sub">${esc(player.pos)} • ${esc(player.team || "FA")} ${injury}</div></div><span class="detail-rank">#${player.rank}</span></div>
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => navigator.serviceWorker.register("./sw.js?v=6.0.1", { updateViaCache: "none" }).catch(() => {}));
}
