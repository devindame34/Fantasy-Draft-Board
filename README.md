# Fantasy Draft Board v6.2

A phone-first, offline-capable 2026 fantasy football draft board for a 12-team, half-PPR, one-QB league. The ADP audit is dated September 4, 2026.
...
- Audited all 250 ADPs against CBS, ESPN, Fantrax, Sleeper and Yahoo managed-league markets.
- Replaced 102 high- or medium-confidence ADPs that materially changed draft timing.
- Manually corrected five additional low-confidence cases with independent support: Sam LaPorta, RJ Harvey, Jaxson Dart, Dallas Goedert and T.J. Hockenson.
- Preserved the news-adjusted estimates for MarShawn Lloyd, Josh Jacobs, Mike Washington Jr. and Jordyn Tyson instead of blindly replacing them with a platform median.
- Recalculated each corrected player's expected round, market range and probability spread so “Will He Make It Back?” uses the same market center shown on the card.
- Added a team filter and retained team abbreviations on every player card and detail sheet.
- Updated every visible version marker and the offline cache to v6.2.
...
Market timing now centers high- and medium-confidence players on the median of CBS, ESPN, Fantrax, Sleeper and Yahoo managed-league ADP, sourced through [4for4's multi-site ADP table](https://www.4for4.com/adp). [Fantasy Football Calculator](https://fantasyfootballcalculator.com/adp/half-ppr) and [FFToday](https://www.fftoday.com/rankings/26-adp-half-ppr.html) remain independent checks for current room behavior and volatility. Best-ball, FFPC and NFFC drafts are not mixed into the managed-league center.

`ADP~` marks a manual late-news adjustment. `ECR` means usable market ADP was too thin, so the app retains the expert consensus as a visibly labeled low-confidence proxy.
